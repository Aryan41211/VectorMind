"""Resume training from checkpoint with memory queue enabled.

Purpose: Resume training from the best checkpoint (Epoch 6) with the
memory queue properly enabled to continue training to convergence.

Usage:
    python scripts/resume_training.py
    python scripts/resume_training.py --epochs 14 --lr 1e-3
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="VectorMind Phase 4: Resume Training"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=14,
        help="Number of epochs to train (default: 14, total 20).",
    )
    parser.add_argument(
        "--lr",
        type=float,
...
def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info("VectorMind Phase 4 -- Resume Training with Memory Queue")
    logger.info("=" * 60)
    ...
    logger.info("=" * 70)
    logger.info("Phase 4 Training Complete (with Memory Queue)")
    logger.info("=" * 70)
